...
export class CurrencyHealthService {
  ...
  async checkHealth(): Promise<Record<string, SourceHealth>> {
    const [primary, fallback] = await Promise.all([
      ...
      this.probe('Parser-CBRF', () =>
        this.parseService.parseCbRFCurrencyRate(),
      ),
    ]);
  ...
  private checkPrimary(): Promise<unknown> {
    const url =
      ...
      '/latest/USD';
    return this.resilientHttpService.fetchWithRetry<any>(url, 1);
  }
  private async probe(
    source: string,
    fn: () => Promise<unknown>,
    ...
    return {
      ...
      availability:
        stat.totalChecks > 0
          ? Number(
              (
                ((stat.totalChecks - stat.totalFailures) / stat.totalChecks) *
                100
              ).toFixed(2),
            )
          : 100,
