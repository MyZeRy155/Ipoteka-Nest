...
| `GET` | `/currency/compare/rub`           | Сравнение двух источников курсов к RUB (`Exchange_API` и скрейпер cbr.ru): доступность и дата обновления данных по каждому источнику, плюс какой из них актуальнее (`moreActual`, `null` если сравнить нельзя — источник недоступен). Только для RUB, так как cbr.ru отдаёт курсы исключительно к рублю. |
| `GET` | `/currency/health`                | Активная проверка доступности обоих источников курсов (`Exchange_API`, `Parser-CBRF`) с накопительной метрикой: статус, задержка, доля успешных проверок за всё время работы приложения. |
